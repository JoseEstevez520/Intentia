...
public class NpcManager {
    ...
    public void loadFromTiledLayer(TiledMap map, String mapPath) {
        for (NPC npc : npcs) {
            npc.dispose();
        }
        npcs.clear();
    ...
    private void cargarNPCsPorDefecto() {
        npcs.add(new NPC("abuelo", "npc_abuelo_hola", 320, 320, NPC.MovementType.PATROL_H, 40f, 3f,
            "sprites/npc/abuelo_walk.png", "sprites/npc/abuelo_idle.png"));
        npcs.add(new NPC("padre", "npc_padre_hola", 240, 240, NPC.MovementType.STATIC, 0, 0,
            "sprites/npc/hombre_walk.png", "sprites/npc/hombre_idle.png"));
        npcs.add(new NPC("madre", "npc_madre_hola", 400, 240, NPC.MovementType.WANDER, 30f, 0,
    ...
    public NPC getInteractiveNPC(Vector2 playerPos) {
        Rectangle playerBounds = new Rectangle(playerPos.x, playerPos.y, 16, 24);
        NPC closest = null;
        float minDist = Float.MAX_VALUE;
        for (NPC npc : npcs) {
            if (npc.getState() == NPC.State.TALKING) continue;
            if (playerBounds.overlaps(npc.getInteractionBounds())) {
